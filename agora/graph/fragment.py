...
def _extract_tp_from_plan(plan):
    """
    ...
    return {tpn: process_tp_node(tpn) for tpn in
...
class Fragment(Graph):
    ...
    def gen(self, *tps):
        ...
        plan = gen_dict.get('plan', None)
        tps = _extract_tp_from_plan(plan) if plan is not None else None
        return plan, self._produce(gen_dict['generator'], tps=tps)
    ...
    def _produce(self, gen, tps=None):
        ...
        for context, s, p, o in gen:
            ...
            self.add((s, p, o))
            subject = context[0] if tps is None else tps[context][0]
    ...
    def query(self, query_object, **kwargs):
        result = super(Fragment, self).query(query_object, processor='agora',
                                             result='agora', **kwargs)
